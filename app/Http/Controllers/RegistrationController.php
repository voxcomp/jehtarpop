...
// 		}
    ...
    public function saveCompany(Request $request, $path) {
		...
		$registration['registrants'] = array();
    ...
    public function registrantShow($path = 'event') {
		...
		switch($path) {
			case 'trade':
				...
				break;
			case 'event': $classes = Event::select('id','name')->where('event',1)->get()->pluck('name','id')->toArray(); break;
			case 'training': $classes = Event::select('id','name')->where('event',0)->get()->pluck('name','id')->toArray(); break;
			case 'online': $classes = OnlineCourse::distinct()->get()->pluck('courseid', 'id')->toArray(); break;
			case 'correspondence': $classes = CorrespondenceCourse::distinct()->where('trade','<>','')->get()->pluck('tradedesc','trade')->toArray(); break; break;
		}
...
*/
    ...
    public function individualShow($path) {
		if(!$this->settings->get($path.'registration','status')) {
			...
			switch($path) {
				case 'trade': $classes = Course::distinct()->get()->pluck('tradedesc','trade')->toArray(); break;
				case 'event': $classes = Event::select('id','name')->where('event',1)->get()->pluck('name','id')->toArray(); break;
				case 'training': $classes = Event::select('id','name')->where('event',0)->get()->pluck('name','id')->toArray(); break;
