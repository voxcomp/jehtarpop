...
class PaymentController extends Controller
{
	...
	private function saveRegistration($path, $payment, $registration) {
        ...
        $reg->registrantcount = sizeof($registration['registrants']);
	...
	public function paymentPage($path) {
	    if(session()->has($path.'registration')) {
			...
			if($total>0) {
				...
				if(isset($registration['registrants']) && sizeof($registration['registrants'])>0) {
					$payment = [
						"total"=>0,
						"due"=>0,
						"paid"=>0,
	            ...
	            if($response->isSuccessful()) {
	                ...
	                if(!$isPaymentExist)
	                {
		                ...
		                switch(substr($newCard->card_number,0,1)) {
			                case '3': $payment['cardtype'] = "American Express"; break;
			                case '4': $payment['cardtype'] = "Visa"; break;
			                case '5': $payment['cardtype'] = "MasterCard"; break;
			                case '6': $payment['cardtype'] = "Discover"; break;
		                }
						$reg = $this->saveRegistration($path,$payment,$registration);
						$payee = [
							"name" => $request->input('firstname')." ".$request->input('lastname'),
